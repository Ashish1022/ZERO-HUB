        ...
        <>
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
                <div className="flex flex-col gap-6">
                    <div className="relative aspect-square overflow-hidden rounded-2xl border border-tpl-line bg-white shadow-sm">
                        {data.badge && (
                        ...
                        {data.images?.map((image, i) => (
                            <div key={i}
                                className={`relative aspect-square overflow-hidden rounded-xl border bg-white cursor-pointer transition-all duration-200 hover:scale-105 ${selectedImageIndex === i
                                    ? "border-tpl-accent ring-2 ring-tpl-accent shadow-md"
                                    : "border-tpl-line hover:border-tpl-accent"
                                    }`}
                            ...
                            >
                                ...
                                {selectedImageIndex === i && (
                                    <div className="absolute inset-0 bg-tpl-accent-soft bg-opacity-20" />
                                )}
                            </div>
                        ))}
                    </div>
                ...
                <div className="flex flex-col gap-6">
                    <div>
                        <h1 className="text-3xl font-bold text-tpl-fg mb-4 md:text-4xl">{data.name}</h1>
                                ...
                                {[...Array(5)].map((_, i) => (
                                    <StarIcon
                                        ...
                                        className={`h-5 w-5 ${i < Math.floor(data.reviewRating)
                                            ? "fill-tpl-accent text-tpl-accent"
                                            : i < data.reviewRating
                                                ? "fill-tpl-accent text-tpl-accent [clip-path:inset(0_50%_0_0)]"
                                                : "text-tpl-fg-subtle"
                                            }`}
                                    />
                                ))}
                            </div>
                            <span className="text-tpl-fg-muted">
                                {data.reviewRating} ({data.reviewCount} reviews)
                            </span>
                        </div>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="text-3xl font-bold text-tpl-fg">
                            {formatPrice(data.pricing.compareAtPrice)}
                        </div>
                        {data.pricing.price && (
                            <div className="text-xl text-tpl-fg-subtle line-through">{formatPrice(data.pricing.price)}</div>
                        )}
                    ...
                    <div className="space-y-6">
                        <div className="flex flex-col gap-4 sm:flex-row">
                            <Button
                                size="lg"
                                className="flex-1 bg-tpl-primary hover:bg-tpl-primary-hover text-white rounded-full py-6 transition-all duration-300 transform hover:scale-105 cursor-pointer"
                            ...
                            {isProductInWishlist(data.id) ? (
                                <Button
                                    variant="outline"
                                    size="lg"
                                    className="border-tpl-line-strong hover:border-red-500 hover:bg-red-50 hover:text-red-600 rounded-full py-6 transition-all duration-300 cursor-pointer"
                                    onClick={() => {
                            ...
                            <Button
                                variant="outline"
                                size="icon"
                                className="h-14 w-14 border-tpl-line-strong hover:border-tpl-primary hover:bg-tpl-primary hover:text-tpl-primary-fg rounded-full transition-all duration-300 cursor-pointer"
                                onClick={() => {
                    ...
                    <div className="rounded-2xl border border-tpl-line bg-linear-to-br from-tpl-accent-soft to-tpl-surface p-6">
                        <div className="space-y-4">
                            <div className="flex items-center gap-3">
                                <div className="rounded-full bg-tpl-accent-soft p-2">
                                    <Truck className="h-5 w-5 text-tpl-accent" />
                                </div>
                                <div>
                                    <p className="font-medium text-tpl-fg">Free Shipping</p>
                                    <p className="text-sm text-tpl-fg-muted">Delivery in 3-5 business days</p>
                                </div>
                            </div>
                            <Separator className="my-3" />
                            <div className="flex items-center gap-3">
                                <div className="rounded-full bg-tpl-accent-soft p-2">
                                    <ShieldCheck className="h-5 w-5 text-tpl-accent" />
                                </div>
                                <div>
                                    <p className="font-medium text-tpl-fg">2-Year Warranty</p>
                                    <p className="text-sm text-tpl-fg-muted">Shop with confidence</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div>
                        <h3 className="mb-4 text-xl font-bold text-tpl-fg">Description</h3>
                        {data.description ? (
                            <p className="text-tpl-fg-muted leading-relaxed" >{data.description}</p>
                        ) : (
                            <p className="text-tpl-fg-muted">No description provided.</p>
                        )}
                    </div>
                </div>
            </div>
            <div className="mt-16">
                <Tabs defaultValue="specifications" className="w-full">
                    <TabsList className="w-full justify-start bg-tpl-surface-muted rounded-full p-1">
                                ...
                                data.specifications.map((spec) => (
                                    <div key={spec.value}>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 px-6 py-4">
                                            <div className="font-medium text-tpl-fg">{spec.name}</div>
                                            <div className="text-tpl-fg-muted">{spec.value}</div>
                                        </div>
                                        <Separator />
                                    </div>
                                ))
                            ) : (
                                <div className="px-4 py-8 text-center text-tpl-fg-subtle">
