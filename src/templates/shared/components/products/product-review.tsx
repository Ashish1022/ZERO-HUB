            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div>
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-tpl-fg-muted">({reviewCount} reviews)</span>
                    </div>
                </div>
                    ...
                    {[5, 4, 3, 2, 1].map((stars) => {
                        return (
                            <div key={stars} className="flex items-center gap-2 text-sm">
                                ...
                                <StarIcon className="h-3 w-3 fill-tpl-fg text-tpl-fg" />
                                <div className="flex-1 bg-tpl-surface-muted rounded-full h-2">
                                    <div className="bg-tpl-primary h-2 rounded-full" style={{ width: `${ratingDistribution[stars]}%` }} />
                                </div>
                                <span className="w-8 text-tpl-fg-muted">{ratingDistribution[stars]}</span>
                            </div>
                        )
                    })}
                </div>
                                ...
                                <div className="mt-1 flex items-center gap-2">
                                    <div className="flex">
                                        {[...Array(5)].map((_, i) => (
                                            <StarIcon
                                                key={i}
                                                className={`h-4 w-4 ${i < review.rating ? "fill-tpl-fg text-tpl-fg" : "text-tpl-fg-subtle"}`}
                                            />
                                        ))}
                                    </div>
                                    <span className="text-sm text-tpl-fg-muted">{new Date(review.createdAt).toLocaleDateString()}</span>
                                </div>
                            </div>
                        </div>
                        <h4 className="mt-2 font-medium">{review.title}</h4>
                        <p className="mt-1 text-tpl-fg-muted">{review.description}</p>
                    </div>
                ))}
            </div>
        </div>
