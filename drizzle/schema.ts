import { pgEnum, pgTable, serial, integer, varchar, timestamp, jsonb, numeric, boolean, index, uniqueIndex, foreignKey, primaryKey, pgView, bigint } from "drizzle-orm/pg-core"
...
export const categories = pgTable("categories", {
	...
	thumbnailId: integer("thumbnail_id").notNull().references(() => media.id, { onDelete: "set null" } ),
	seoTitle: varchar("seo_title"),
	seoDescription: varchar("seo_description"),
	seoKeywords: varchar("seo_keywords"),
	seoOgImageId: integer("seo_og_image_id").references(() => media.id, { onDelete: "set null" } ),
	updatedAt: timestamp("updated_at", { precision: 3, withTimezone: true }).default(sql`now()`).notNull(),
	...
	index("tenantSlug_featured_idx").using("btree", table.tenantSlug.asc().nullsLast(), table.featured.asc().nullsLast()),
	uniqueIndex("tenantSlug_slug_1_idx").using("btree", table.tenantSlug.asc().nullsLast(), table.slug.asc().nullsLast()),
]);
...
export const categorySalesSummary = pgTable("category_sales_summary", {
	...
	index("category_sales_summary_created_at_idx").using("btree", table.createdAt.asc().nullsLast()),
	index("category_sales_summary_tenant_idx").using("btree", table.tenantId.asc().nullsLast()),
	index("category_sales_summary_updated_at_idx").using("btree", table.updatedAt.asc().nullsLast()),
]);
...
export const orders = pgTable("orders", {
	...
	tenantId: integer("tenant_id").references(() => tenants.id, { onDelete: "set null" } ),
	name: varchar().notNull(),
	customerId: integer("customer_id").notNull().references(() => customers.id, { onDelete: "set null" } ),
...
export const ordersOrderItems = pgTable("orders_order_items", {
	...
	parentId: integer("_parent_id").notNull().references(() => orders.id, { onDelete: "cascade" } ),
...
export const payloadKv = pgTable("payload_kv", {
	...
	uniqueIndex("payload_kv_key_idx").using("btree", table.key.asc().nullsLast()),
]);
...
export const payloadLockedDocuments = pgTable("payload_locked_documents", {
	...
	index("payload_locked_documents_created_at_idx").using("btree", table.createdAt.asc().nullsLast()),
	index("payload_locked_documents_global_slug_idx").using("btree", table.globalSlug.asc().nullsLast()),
	index("payload_locked_documents_updated_at_idx").using("btree", table.updatedAt.asc().nullsLast()),
]);
...
export const payloadLockedDocumentsRels = pgTable("payload_locked_documents_rels", {
	...
	tenantsId: integer("tenants_id").references(() => tenants.id, { onDelete: "cascade" } ),
	productsId: integer("products_id").references(() => products.id, { onDelete: "cascade" } ),
	tagsId: integer("tags_id").references(() => tags.id, { onDelete: "cascade" } ),
	categoriesId: integer("categories_id").references(() => categories.id, { onDelete: "cascade" } ),
	reviewsId: integer("reviews_id").references(() => reviews.id, { onDelete: "cascade" } ),
	subscriptionPlansId: integer("subscription_plans_id").references(() => subscriptionPlans.id, { onDelete: "cascade" } ),
	subscriptionsId: integer("subscriptions_id").references(() => subscriptions.id, { onDelete: "cascade" } ),
	customersId: integer("customers_id").references(() => customers.id, { onDelete: "cascade" } ),
	ordersId: integer("orders_id").references(() => orders.id, { onDelete: "cascade" } ),
	categorySalesSummaryId: integer("category_sales_summary_id").references(() => categorySalesSummary.id, { onDelete: "cascade" } ),
	productsSalesSummaryId: integer("products_sales_summary_id").references(() => productsSalesSummary.id, { onDelete: "cascade" } ),
	monthlySalesSummaryId: integer("monthly_sales_summary_id").references(() => monthlySalesSummary.id, { onDelete: "cascade" } ),
}, (table) => [
	index("payload_locked_documents_rels_categories_id_idx").using("btree", table.categoriesId.asc().nullsLast()),
	index("payload_locked_documents_rels_category_sales_summary_id_idx").using("btree", table.categorySalesSummaryId.asc().nullsLast()),
	index("payload_locked_documents_rels_customers_id_idx").using("btree", table.customersId.asc().nullsLast()),
	index("payload_locked_documents_rels_media_id_idx").using("btree", table.mediaId.asc().nullsLast()),
	index("payload_locked_documents_rels_monthly_sales_summary_id_idx").using("btree", table.monthlySalesSummaryId.asc().nullsLast()),
	index("payload_locked_documents_rels_order_idx").using("btree", table.order.asc().nullsLast()),
	index("payload_locked_documents_rels_orders_id_idx").using("btree", table.ordersId.asc().nullsLast()),
	index("payload_locked_documents_rels_parent_idx").using("btree", table.parentId.asc().nullsLast()),
	index("payload_locked_documents_rels_path_idx").using("btree", table.path.asc().nullsLast()),
	index("payload_locked_documents_rels_products_id_idx").using("btree", table.productsId.asc().nullsLast()),
	index("payload_locked_documents_rels_products_sales_summary_id_idx").using("btree", table.productsSalesSummaryId.asc().nullsLast()),
	index("payload_locked_documents_rels_reviews_id_idx").using("btree", table.reviewsId.asc().nullsLast()),
	index("payload_locked_documents_rels_subscription_plans_id_idx").using("btree", table.subscriptionPlansId.asc().nullsLast()),
	index("payload_locked_documents_rels_subscriptions_id_idx").using("btree", table.subscriptionsId.asc().nullsLast()),
	index("payload_locked_documents_rels_tags_id_idx").using("btree", table.tagsId.asc().nullsLast()),
	index("payload_locked_documents_rels_tenants_id_idx").using("btree", table.tenantsId.asc().nullsLast()),
	index("payload_locked_documents_rels_users_id_idx").using("btree", table.usersId.asc().nullsLast()),
]);
...
export const products = pgTable("products", {
	...
	inventoryLowStockThreshold: numeric("inventory_low_stock_threshold", { mode: 'number' }).default(5),
	inventoryAllowBackorders: boolean("inventory_allow_backorders").default(false),
	categoryId: integer("category_id").notNull().references(() => categories.id, { onDelete: "set null" } ),
	seoTitle: varchar("seo_title"),
	seoDescription: varchar("seo_description"),
	seoKeywords: varchar("seo_keywords"),
	featured: boolean().default(false),
	...
	index("tenantSlug_name_idx").using("btree", table.tenantSlug.asc().nullsLast(), table.name.asc().nullsLast()),
	uniqueIndex("tenantSlug_slug_idx").using("btree", table.tenantSlug.asc().nullsLast(), table.slug.asc().nullsLast()),
]);

export const productsImages = pgTable("products_images", {
	order: integer("_order").notNull(),
	parentId: integer("_parent_id").notNull().references(() => products.id, { onDelete: "cascade" } ),
	id: varchar().primaryKey(),
	imageId: integer("image_id").notNull().references(() => media.id, { onDelete: "set null" } ),
...
export const productsSalesSummary = pgTable("products_sales_summary", {
	...
	index("products_sales_summary_created_at_idx").using("btree", table.createdAt.asc().nullsLast()),
	index("products_sales_summary_tenant_idx").using("btree", table.tenantId.asc().nullsLast()),
	index("products_sales_summary_updated_at_idx").using("btree", table.updatedAt.asc().nullsLast()),
]);
...
export const productsVariantsOptions = pgTable("products_variants_options", {
	order: integer("_order").notNull(),
	parentId: varchar("_parent_id").notNull().references(() => productsVariants.id, { onDelete: "cascade" } ),
	id: varchar().primaryKey(),
	...
	index("products_variants_options_parent_id_idx").using("btree", table.parentId.asc().nullsLast()),
]);
...
export const subscriptionPlans = pgTable("subscription_plans", {
	...
	uniqueIndex("subscription_plans_razorpay_plan_id_idx").using("btree", table.razorpayPlanId.asc().nullsLast()),
...
export const subscriptionPlansFeatures = pgTable("subscription_plans_features", {
	order: integer("_order").notNull(),
	parentId: integer("_parent_id").notNull().references(() => subscriptionPlans.id, { onDelete: "cascade" } ),
	id: varchar().primaryKey(),
	feature: varchar().notNull(),
}, (table) => [
	index("subscription_plans_features_order_idx").using("btree", table.order.asc().nullsLast()),
	index("subscription_plans_features_parent_id_idx").using("btree", table.parentId.asc().nullsLast()),
]);

export const subscriptions = pgTable("subscriptions", {
	id: serial().primaryKey(),
	tenantId: integer("tenant_id").notNull().references(() => tenants.id, { onDelete: "set null" } ),
	planId: integer("plan_id").notNull().references(() => subscriptionPlans.id, { onDelete: "set null" } ),
	...
	uniqueIndex("subscriptions_razorpay_subscription_id_idx").using("btree", table.razorpaySubscriptionId.asc().nullsLast()),
...
export const tenants = pgTable("tenants", {
	...
	subscriptionSubscriptionStartDate: timestamp("subscription_subscription_start_date", { precision: 3, withTimezone: true }),
	subscriptionSubscriptionEndDate: timestamp("subscription_subscription_end_date", { precision: 3, withTimezone: true }),
	subscriptionTrialStartDate: timestamp("subscription_trial_start_date", { precision: 3, withTimezone: true }),
	subscriptionTrialEndDate: timestamp("subscription_trial_end_date", { precision: 3, withTimezone: true }),
	...
	index("tenants_subscription_subscription_subscription_status_idx").using("btree", table.subscriptionSubscriptionStatus.asc().nullsLast()),
...
export const users = pgTable("users", {
	...
	resetPasswordExpiration: timestamp("reset_password_expiration", { precision: 3, withTimezone: true }),
	salt: varchar(),
	hash: varchar(),
...
export const usersSessions = pgTable("users_sessions", {
	order: integer("_order").notNull(),
	parentId: integer("_parent_id").notNull().references(() => users.id, { onDelete: "cascade" } ),
	id: varchar().primaryKey(),
...
export const vProductPerformanceByTenant = pgView("v_product_performance_by_tenant", {	tenantId: integer("tenant_id"),
...
}).as(sql`SELECT tenant_id, product_name, sum(gross_sales) AS total_gross_sales, sum(net_sales) AS total_net_sales, sum(cost_price) AS total_cost_price, sum(total_items_sold) AS total_items_sold, sum(total_orders) AS total_orders, row_number() OVER (PARTITION BY tenant_id ORDER BY (sum(gross_sales)) DESC) AS product_rank_high, row_number() OVER (PARTITION BY tenant_id ORDER BY (sum(gross_sales))) AS product_rank_low FROM products_sales_summary GROUP BY tenant_id, product_name`);

export const vTenantMonthlySales = pgView("v_tenant_monthly_sales", {	tenantId: integer("tenant_id"),
	month: varchar(),
	totalGrossSales: numeric("total_gross_sales"),
	totalNetSales: numeric("total_net_sales"),
	totalOrders: numeric("total_orders"),
	averageOrderValue: numeric("average_order_value"),
}).as(sql`SELECT tenant_id, month, sum(gross_sales) AS total_gross_sales, sum(net_sales) AS total_net_sales, sum(total_orders) AS total_orders, sum(gross_sales) / NULLIF(sum(total_orders), 0::numeric) AS average_order_value FROM monthly_sales_summary GROUP BY tenant_id, month`);

export const vTopCategoriesByTenant = pgView("v_top_categories_by_tenant", {	tenantId: integer("tenant_id"),
